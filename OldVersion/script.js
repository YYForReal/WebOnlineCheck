...
inputs[0].onchange = function () {
    ...
    reader.onload = function () {
        source_txt = reader.result.split('');
        //过滤 回车符号 和 换行符号
        source_txt = source_txt.filter((char)=>{
            return char != '\r' && char != '\n'; 
        })
        console.log(source_txt);
        result_text.innerText = "读取txt文本成功\r\n";
        a.textContent = source_txt.join("");
        if(result_txt!=null){
            checkFirst();
            result_text.innerText += "---------------- \r\n";
            checkSecond();    
...
inputs[1].onchange = function () {
    ...
    // 文件读取完成后，获取读取的结果
    reader.onload = function () {
        result_txt = reader.result;
        
        
        iframe.src = result_txt;

        var iwindow = iframe.contentWindow; //获取iframe的window对象
        var idoc = iwindow.document; //获取iframe的document对象
        idoc.open();
        idoc.write(result_txt);
        idoc.close();
        
        // 取body标签后的内容
        let index = result_txt.indexOf('body');
        if(index!=-1){
            result_txt = result_txt.slice(index);
        }

        result_text.innerText = "读取html文件成功\r\n";
        // 去除标签的影响
        result_txt = delHtmlTag(result_txt);
        
        b.textContent = result_txt;
        if(source_txt != null){
            checkFirst();
            result_text.innerText += "---------------- \r\n";
            checkSecond();  
...
function checkFirst(){
    ...
    while(nowIndex < source_txt.length){
        let checkText = source_txt[nowIndex];
        searchResult = result_txt.indexOf(checkText,searchIndex);
        if(searchResult == -1){
    ...
    if(isError){
        result_text.innerText += "第一次文字校验失败：\r\n可能原因：字符错误、遗漏字符 或 字符顺序与比对文本文件不匹配。\r\n"
        return false;
    }else{
        result_text.innerText += "第一次文字校验成功\r\n";
        return true;
    }
}
 ...
 */
 function checkSecond(){
    let html_text = delHtmlTag(result_txt);
    let comp_text = delHtmlTag(source_txt.join(''));
    console.log("html:")
    console.log(html_text);
    console.log("text:")
    console.log(comp_text);
    
    if(html_text.indexOf(comp_text) == -1){
        result_text.innerText += "第二次文字校验失败：\r\n可能原因：字符错误、遗漏字符、字符顺序与比对文本文件不匹配、html存在多余内容\r\n";
        return false;
    }else{
        result_text.innerText += "第二次文字校验成功\r\n";
        return true;
    }
}
